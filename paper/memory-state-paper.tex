...
The reported numbers are for retrieval tasks derived from LongMemEval
and PrefEval data, not their official end-to-end protocols. In
particular, we do not measure LongMemEval answer accuracy or PrefEval
generation/classification accuracy. The LongMemEval slice is also
strongly composition-biased because the source file is ordered by
question type. It contains every single-session-user item and 30
multi-session items, but no temporal-reasoning, knowledge-update,
single-session-assistant, or single-session-preference items. It cannot
represent LongMemEval as a whole. The 94 scored questions also mean that
one question moves the metric by about 1.1 percentage points.
Differences of one or two questions between the reported systems and the
embedding baselines should be read as parity, not as ordering.
