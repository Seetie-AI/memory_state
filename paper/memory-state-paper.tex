...
separately trained retriever. The contribution is this
...
the same embedding baseline. At roughly comparable parameter scale,
hidden states from a general Qwen3.5-9B model, with no trained retriever,
reach parity with a dedicated Qwen3-Embedding-8B model on these derived
retrieval tasks. A matched 2B-to-9B
...
We report three things. First, hidden-state keys from a general 9B
generative model reach comparable-scale parity with a dedicated 8B
embedding model on two benchmark-derived memory retrieval tasks. Second,
three simple memory facets selected on conversational
...
The comparable-scale embedding baseline is not generation-matched.
Memory State uses the newer Qwen3.5 family, while the dedicated
embedding baseline is built on Qwen3, which favors our side of the
comparison.
...
representation that can be used to find that page again. At roughly
comparable parameter scale, this representation reached parity with a
dedicated embedding model on retrieval tasks derived from two different
memory benchmarks, without training a retriever. It also improved with
model size under matched conditions.

In deployment there is no second model to run, and the memory index may
improve as the reasoning model does. The memory an agent needs may
already be present in the act of understanding, waiting to be read out
rather than rebuilt.
